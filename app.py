import streamlit as st
import requests
import pandas as pd
from datetime import datetime

# --- CONFIGURAZIONE ---
API_KEY = "adf7b41bd4a85edbf0d28b46c647b3d7"
HEADERS = {'x-rapidapi-key': API_KEY, 'x-rapidapi-host': 'v3.football.api-sports.io'}

# Leghe con medie gol reali per calibrare la formula
LEAGUES = {
    135: "Serie A", 136: "Serie B", 39: "Premier League", 
    78: "Bundesliga", 88: "Eredivisie", 140: "La Liga", 
    61: "Ligue 1", 207: "Super League (CH)", 208: "Challenge League"
}

st.set_page_config(page_title="REAL XG PREDICTOR", layout="wide")

def get_stats(l_id, t_id):
    """Recupera GF e GS reali. Se fallisce, restituisce 1.1 per non bloccare il calcolo."""
    try:
        url = f"https://v3.football.api-sports.io/teams/statistics?league={l_id}&season=2024&team={t_id}"
        res = requests.get(url, headers=HEADERS, timeout=5).json()
        if 'response' in res and res['response']:
            g = res['response']['goals']
            f = float(g['for']['average']['total']) if g['for']['average']['total'] else 1.1
            s = float(g['against']['average']['total']) if g['against']['average']['total'] else 1.2
            return f, s
    except:
        pass
    return 1.1, 1.2

# --- LOGIN ---
if "auth" not in st.session_state:
    st.session_state["auth"] = False

if not st.session_state["auth"]:
    st.title("🔐 Accesso Sistema")
    pwd = st.text_input("Inserisci Password", type="password")
    if st.button("SBLOCCA"):
        if pwd == "DAJE80":
            st.session_state["auth"] = True
            st.rerun()
    st.stop()

# --- INTERFACCIA ---
st.title("📊 Analizzatore xG Reale (V. Professionale)")

if st.button("🚀 GENERA ANALISI SCIENTIFICA"):
    with st.spinner('Scansione database squadre...'):
        today = datetime.now().strftime('%Y-%m-%d')
        url_fix = f"https://v3.football.api-sports.io/fixtures?date={today}"
        
        try:
            res_fix = requests.get(url_fix, headers=HEADERS, timeout=10).json()
            results = []
            
            if 'response' in res_fix and res_fix['response']:
                fixtures = [f
